...
**丢弃法（dropout）** 是另一种直观而有效的正则化手段：训练时**随机丢弃**隐藏层的一部分神经元（将其输出置 0），迫使网络不能依赖任何单个神经元，从而学习更鲁棒的冗余表示。教材实现的是现代通用的**倒置丢弃法（inverted dropout）**：
...
**倒置丢弃法的关键**：除以 `keep_prob` 使输出的期望在训练与预测时一致（被保留的神经元输出放大 $1/\text{keep\_prob}$ 倍），因此**预测（推理）时不再需要任何调整**——只需关闭 dropout。
