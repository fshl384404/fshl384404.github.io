...
$$\begin{aligned}
f_t &= \sigma(W_f[h_{t-1}, x_t] + b_f)   &&\text{遗忘门：决定保留多少旧记忆}\\
i_t &= \sigma(W_i[h_{t-1}, x_t] + b_i)   &&\text{输入门：决定写入多少新信息}\\
o_t &= \sigma(W_o[h_{t-1}, x_t] + b_o)   &&\text{输出门：决定输出多少记忆}\\
\tilde{C}_t &= \tanh(W_c[h_{t-1}, x_t] + b_c) &&\text{候选记忆}\\
C_t &= f_t \odot C_{t-1} + i_t \odot \tilde{C}_t &&\text{更新细胞状态}\\
h_t &= o_t \odot \tanh(C_t)              &&\text{更新隐状态}
\end{aligned}$$
...
学习结构的价值在于能**公平地比较**：同一数据集、同一评估协议下，"改一个变量"看差异。汇总本文（及第 4 篇）训练过的模型：

| 任务 | 模型 | 测试准确率 / 困惑度（参考值） | 关键差异 |
|---|---|---|---|
| Fashion-MNIST 图像分类 | MLP（第 4 篇） | 0.845 | 基线 |
...
阅读这张表的方式：每一行都是前一行上"只改一个变量"的结果——从 MLP 到 LeNet 是"换成卷积"，从 LeNet 到 ResNet 是"加残差"，从 RNN 到 LSTM 是"加门控"，从 LSTM 到 Transformer 是"换成自注意力"。结论要归因于**被改动的那个设计**，而不是训练时长、学习率等其它因素——这正是第 3 篇"诊断驱动"方法论在模型结构层面的延伸。比较时还需注意：不同来源的数值（不同实现、不同训练轮数、不同框架版本）不能直接对比，必须在自己环境里重跑（衔接第 2 篇评估指标的讨论）。
